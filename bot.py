...
def setup_welcome(client):
    ...
    async def on_member_join(member):
        ...
        if ch:
            ...
            em.set_author(name=str(member), icon_url=safe_avatar_url(member))
...
class WarStatsModal(Modal):
    def __init__(self, msg_id, selected_class):
        # 45 character limit on Modal titles
        super().__init__(title=f"Stats: {selected_class}"[:45])
        self.msg_id = msg_id
        self.selected_class = selected_class
        
        # Pull required stats dynamically based on the selected class
    ...
    async def callback(self, interaction: Interaction):
        ...
        for i, stat in enumerate(self.stat_fields):
            ...
            # Validation: Strip commas and percents to check if it's numerical
            clean_val = val.replace("%", "").replace(",", "")
            try:
                float(clean_val) # Verify it can be converted to a number
                # Automatically add % back for visual clarity if they forgot to type it
                if "%" in stat and "%" not in val:
                    val += "%" 
            except ValueError:
...
class WarClassSelect(Select):
    ...
    async def callback(self, interaction: Interaction):
        ...
        # Pop the dynamically generated Modal based on class selection
...
class WarGenerateButton(Button):
    def __init__(self):
        super().__init__(label="Generate 20-Man Roster", style=ButtonStyle.primary, custom_id="war_generate_btn", emoji="📋", row=3)

    async def callback(self, interaction: Interaction):
        if not is_war_role(interaction.user):
            return await interaction.response.send_message("⛔ Restricted to War Role.", ephemeral=True)
        ...
        for uid, info in roster.items():
            ...
            if c == "Sword / Shield":
                tanks.append((uid, parse_stat(stats.get("MDP", 0))))
            elif c == "Life Staff":
                healers.append((uid, parse_stat(stats.get("Heal Multiplier (%)", 0))))
            elif c == "Necromancer":
                necros.append((uid, parse_stat(stats.get("HP", 0))))
            elif c in ["Elementalist", "Dual Crossbow"]:
                dps.append((uid, parse_stat(stats.get("AP", 0))))

        tanks.sort(key=lambda x: x[1], reverse=True)
        healers.sort(key=lambda x: x[1], reverse=True)
        necros.sort(key=lambda x: x[1], reverse=True)
        dps.sort(key=lambda x: x[1], reverse=True)

        top_tanks = tanks[:4]
        top_healers = healers[:4]
        top_necros = necros[:4]
        top_dps = dps[:8]
