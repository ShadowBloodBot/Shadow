...
class ShadowControlPanel(View):
    ...
    async def analytics(self, button: Button, interaction: discord.Interaction):
        try:
            from analytics import get_bot_stats  # moved inline to avoid circular crash
            stats = get_bot_stats(self.bot)
            embed = discord.Embed(title="📊 Bot Analytics", color=discord.Color.blue())
            for key, val in stats.items():
                embed.add_field(name=key.capitalize(), value=str(val))
            await interaction.response.send_message(embed=embed, ephemeral=True)
        except Exception as e:
            await interaction.response.send_message(f"⚠️ Analytics failed: {e}", ephemeral=True)
