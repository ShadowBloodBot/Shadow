...
MONSTERS = {
    ...
    5: ["Goblin Scout", "Skeleton Warrior", "Corrupt Guard", "Giant Spider", "Orc"],
    15: ["Troll", "Ogre", "Gargoyle", "Vampire Spawn", "Cursed Armor", "Knight"],
    30: ["Lich", "Demon Soldier", "Shadow Stalker", "Bone Golem", "Hellhound"],
    50: ["Void Walker", "Abyssal Horror", "Fallen Angel", "Dragon", "Void Titan"],
    80: ["Ego Fragment", "Aspect of Greed", "The Usurper", "Mind Flayer", "Demon"]
} 

BIOMES = {
    "Sewers": {
        "range": (1, 20), "color": 0x2ECC71, "emoji": "🤢", 
        "effect": "Toxic: 5% Poison Dmg every 5 turns."
    },
    "Catacombs": {
        "range": (21, 40), "color": 0x95A5A6, "emoji": "💀", 
        "effect": "Darkness: 20% Miss Chance."
    },
    "Magma Core": {
        "range": (41, 60), "color": 0xE74C3C, "emoji": "🌋", 
        "effect": "Heat: Skills cost 5 HP."
    },
    "The Abyss": {
        "range": (61, 80), "color": 0x8E44AD, "emoji": "🔮", 
        "effect": "Despair: Healing reduced by 50%."
    },
    "Court of Shadows": {
        "range": (81, 999), "color": THEME_BLOOD, "emoji": "👑", 
        "effect": "Absolute Power: Enemies have 20% execute chance."
    }
}
...
def _save_tower(): 
    _atomic_write(TOWER_STORE, tower_db) 
...
def get_tower_data(user_id):
    ...
    if uid not in tower_db:
        tower_db[uid] = {
            "floor": 1, "max_floor": 1, "hp": 100, "max_hp": 100, "gold": 0, 
            "checkpoint": 1, "potions": 3, "class": "Warrior", "level": 1, "xp": 0, 
...
def generate_rpg_item(floor, cursed=False):
    if cursed:
        ...
        budget_map = {"Common": 2, "Uncommon": 5, "Rare": 10, "Epic": 20, "Legendary": 40}
        budget = floor + budget_map[rarity]
        
    slot = random.choice(ITEM_SLOTS)
    stats = {}
    possible_stats = ["str", "vit", "agi", "int"]
    
    stat_counts = {"Common": 1, "Uncommon": 2, "Rare": 3, "Epic": 4, "Legendary": 4, "Cursed": 4}
    num_stats = stat_counts[rarity]
    ...
    if cursed:
        ...
        if rarity == "Legendary": 
            prefix = random.choice(['God', 'Titan', 'Dragon', 'Void', 'Emperor'])
            name = f"The {prefix}'s {slot}"

    return {
        "id": str(uuid.uuid4())[:8], "name": name, "rarity": rarity, 
        "slot": slot, "stats": stats, "value": budget * 5
    }

def get_biome(floor):
    for name, data in BIOMES.items():
        if data["range"][0] <= floor <= data["range"][1]: 
            return name, data
...
class LootDropView(View):
    ...
    async def take(self, interaction):
        ...
        msg = f"Looted **{self.item['name']}**."
        await interaction.response.edit_message(embed=view.update_embed("Climbing", msg), view=view)
...
class DarkAltarView(View):
    def __init__(self, user):
        ...
        btn_sac = Button(label="Bleed (-10 Vit, +3 All)", style=ButtonStyle.danger, emoji="🩸")
        btn_sac.callback = self.sacrifice_health
        self.add_item(btn_sac)
        
        btn_gamble = Button(label="Gamble Soul", style=ButtonStyle.primary, emoji="🔮")
    ...
    async def sacrifice_health(self, interaction):
        ...
        save_tower_data(self.user.id, self.data)
        view = TowerGameView(self.user)
        msg = "Your vitality drains, but power surges through you. Power is taken."
        await interaction.response.edit_message(embed=view.update_embed("Pact Sealed", msg), view=view)
    ...
    async def gamble_soul(self, interaction):
        ...
        view = TowerGameView(self.user)
        desc = f"{msg}\nReceived **{item['name']}**."
        await interaction.response.edit_message(embed=view.update_embed("Altar Yields", desc), view=view)

    async def walk_away(self, interaction):
        if interaction.user.id != self.user.id: return
        view = TowerGameView(self.user)
        msg = "You leave the altar untouched. Survival over greed."
        await interaction.response.edit_message(embed=view.update_embed("Exploration", msg), view=view)
...
class TowerGameView(View):
    ...
    def get_inventory_embed(self):
        ...
        if not self.data["inventory"]:
            ...
            lines = [f"• {i['name']}" for i in self.data['inventory'][:5]]
    ...
    def get_shop_embed(self):
        embed = discord.Embed(
            ...
            description="Sanctuary is an illusion. Rest while you can afford it.", 
            color=THEME_GOLD
        )
        gold = self.data['gold']
        souls = self.data.get('souls', 0)
        embed.add_field(name="Reserves", value=f"💰 {gold} Gold\n👻 {souls} Souls")
    ...
    def update_embed(self, title, desc, color=THEME_PRIMARY):
        ...
        if self.mode == "COMBAT" and self.enemy:
            ...
            combat_desc = f"{e_bar} {self.enemy['hp']} HP\n⚠️ **Intent:** {intent}"
            
            embed.add_field(name=f"🆚 {self.enemy['name']}", value=combat_desc, inline=False)
        ...
        s_atk = self.stats['atk']
        s_def = self.stats['vit'] // 2
        gold = self.data['gold']
        stats_disp = f"⚔️{s_atk} 🛡️{s_def} 💰{gold}"
        
        player_desc = f"{p_bar} {self.data['hp']} HP\n{a_bar} Limit Break\n{stats_disp}"
        embed.add_field(
    ...
    def render_main_menu(self):
        ...
        if self.mode == "COMBAT":
            ...
            if self.data["potions"] > 0:
                pot_lbl = f"Elixir ({self.data['potions']})"
                self.add_item(ActionButton("act_pot", pot_lbl, ButtonStyle.success, "🧪", 0))
            ...
            if self.data["inventory"]:
                opts = []
                for i in self.data["inventory"][:25]:
                    lbl = f"{i['name']} ({i['slot']})"
                    opts.append(SelectOption(label=lbl, value=i["id"]))
    ...
    async def equip_callback(self, interaction, val):
        ...
        if to_equip:
            ...
            if current: 
                self.data["inventory"].append(current)
            ...
            msg = "Always arm yourself against betrayal."
            await interaction.edit_original_response(
                embed=self.update_embed("Arsenal Updated", msg), 
                view=self
            )
    ...
    async def wrapper(self, interaction, cid):
        try:
            ...
            if "act_" in cid: 
                ...
                self.render_main_menu()
                await interaction.edit_original_response(
                    embed=self.update_embed("The Vault", "Take stock of your leverage."), 
                    view=self
                )
            elif cid == "nav_back":
                self.mode = "EXPLORE"
                self.render_main_menu()
                await interaction.edit_original_response(
                    embed=self.update_embed("Ascension", "The climb continues."), 
                    view=self
                )
            elif "shop_" in cid: 
                await self.resolve_shop(interaction, cid)
            elif cid in ["nav_climb", "nav_rest"]: 
    ...
    async def resolve_shop(self, interaction, cid):
        if cid == "shop_buy":
            if self.data["gold"] >= 50:
                ...
                self.data["potions"] += 1
                save_tower_data(self.user_id, self.data)
                embed = self.update_embed("Black Market", "Elixir acquired.")
                await interaction.edit_original_response(embed=embed, view=self)
            else: 
                await interaction.followup.send("❌ Need 50 Gold.", ephemeral=True)
            ...
            msg = f"Liquidated {count} assets for {total}g. Wealth is leverage."
            embed = self.update_embed("Black Market", msg)
            await interaction.edit_original_response(embed=embed, view=self)
            ...
            msg = "You leave the false safety of the camp behind."
            embed = self.update_embed("Ascension", msg)
            await interaction.edit_original_response(embed=embed, view=self)
    ...
    async def resolve_nav(self, interaction, cid):
        if cid == "nav_rest":
            if self.data["gold"] >= 100:
                ...
                embed = self.update_embed("💤 Rested", "HP Restored.")
                await interaction.edit_original_response(embed=embed, view=self)
            else: 
            ...
            if self.data["floor"] % 5 == 0 and self.data["floor"] > 1:
                self.mode = "SHOP"
                self.render_main_menu()
                embed = self.update_embed("Black Market", "A neutral zone.")
                await interaction.edit_original_response(embed=embed, view=self)
                return
            ...
            if roll <= 10:
                ...
                desc = "A monolith pulses with energy. Law 28: Enter action with boldness."
                embed = discord.Embed(title="🩸 Dark Altar", description=desc, color=THEME_BLOOD)
                ...
                s_list = [f"• **{k.upper()}:** +{v}" for k,v in item['stats'].items()]
                stats_str = "\n".join(s_list)
                ...
                col = RARITY_COLORS.get(item['rarity'], 0xFFFFFF)
                embed = discord.Embed(title="🎁 Asset Seized", description=desc, color=col)
                await interaction.edit_original_response(embed=embed, view=view)
                
            else: 
                self.start_combat()
                embed = self.update_embed("⚔️ Conflict Initiated", "Show no mercy.")
                await interaction.edit_original_response(embed=embed, view=self)
    ...
    def start_combat(self):
        ...
        intent = random.choice(["Strike", "Heavy Strike"])
        self.enemy = {"name": name, "hp": hp, "max_hp": hp, "power": power, "intent": intent}
        self.combat_log = [f"⚔️ {name} blocks your path!"]
        self.render_main_menu()
    ...
    async def resolve_combat(self, interaction, action):
        ...
        if action == "act_atk":
            ...
            if random.randint(1, 100) <= self.stats["crit_chance"]: 
                dmg = int(dmg * 1.5)
                self.combat_log.append(f"💥 CRITICAL EXECUTION! {dmg} dmg.")
            else: 
                self.combat_log.append(f"🗡️ Strike lands: {dmg} dmg.")
                
            p_dmg = dmg
            self.data["adrenaline"] = min(100, self.data["adrenaline"] + 10)
            
        elif action == "act_def":
            p_block = self.stats["vit"]
            self.combat_log.append(f"🛡️ Guard raised. Mitigating {p_block}.")
            self.data["adrenaline"] = min(100, self.data["adrenaline"] + 5)
            
        elif action == "act_ult":
            p_dmg = self.stats["atk"] * 3
            self.combat_log.append(f"⚡ ABSOLUTE DOMINANCE! {p_dmg} DMG!")
            ...
            self.data["potions"] -= 1
            self.combat_log.append(f"🧪 Restored +{heal} HP.")

        self.enemy["hp"] -= p_dmg
        
        if self.enemy["hp"] > 0:
            e_dmg = self.enemy["power"]
            if self.enemy["intent"] == "Heavy Strike": 
                e_dmg = int(e_dmg * 1.5)
            ...
            self.combat_log.append(f"👾 Enemy strikes for {final_dmg}.")
        ...
        if self.enemy["hp"] <= 0:
            ...
            if self.data["xp"] >= req:
                self.data["xp"] -= req
                self.data["level"] += 1
                self.data["stats"]["str"] += 1
                self.data["stats"]["vit"] += 1
            ...
            msg = f"Obstacle removed.\n+{xp_gain} XP | +{gold_gain} Gold{soul_str}"
            embed = self.update_embed("Target Eliminated", msg)
            await interaction.edit_original_response(embed=embed, view=self)
            ...
            save_tower_data(self.user_id, self.data)
            msg = f"You showed weakness.\nLost {lost_gold} Gold.\nFloor reduced."
            embed = self.update_embed("💀 Broken", msg)
            await interaction.edit_original_response(embed=embed, view=None)
            
        else:
            save_tower_data(self.user_id, self.data)
            embed = self.update_embed("Conflict", "Maintain control...")
            await interaction.edit_original_response(embed=embed, view=self)
...
class TowerCog(commands.Cog):
    ...
    def _load_data(self):
        ...
        if TOWER_STORE.exists():
            try: 
                tower_db = json.loads(TOWER_STORE.read_text())
            except: 
                tower_db = {}
        else: 
            tower_db = {}
    ...
    async def tower(self, ctx):
        try:
            ...
            msg = "You cross the threshold. There is no retreat, only leverage."
            embed = view.update_embed("The Shadow Tower", msg)
            await safe_reply(ctx, embed=embed, view=view)
        except Exception as e:
            traceback.print_exc()
            await safe_reply(ctx, f"⚠️ System Exception: {e}", ephemeral=True)

    @discord.slash_command(name="tower_duel", description="Crush a rival. Steal their gold. Claim souls.")
    async def tower_duel(self, ctx, target: Option(discord.Member, "The rival to crush.")):
        try:
            if ctx.author.id == target.id: 
                return await safe_reply(ctx, "❌ Self-destruction is not a strategy.", ephemeral=True)
            
            u1_data = get_tower_data(ctx.author.id)
            if str(target.id) not in tower_db: 
                return await safe_reply(ctx, "❌ This target is entirely irrelevant.", ephemeral=True)
                
            u2_data = get_tower_data(target.id)
            s1 = get_total_stats(u1_data)
            s2 = get_total_stats(u2_data)
            ...
            if cp1_final > cp2_final:
                ...
                desc = (
                    f"**{ctx.author.display_name}** established absolute dominance.\n"
                    f"Law 15: Crush your enemy totally.\n\n"
                    f"💰 **Plundered:** {stolen_gold} Gold{bounty_str}\n"
                    f"💀 **Gained:** 5 Souls"
                )
                ...
                desc = (
                    f"**{ctx.author.display_name}** overestimated their leverage.\n"
                    f"**{target.display_name}** repelled the assault.\n\n"
                    f"💰 **Lost:** {stolen_gold} Gold\n"
                    f"💀 You have proven your weakness."
                )
    ...
    @discord.slash_command(name="tower_bounty", description="Place a price on a rival's head.")
    async def tower_bounty(self, ctx, target: Option(discord.Member, "The rival to mark."), amount: Option(int, "Gold to place on their head.")):
        try:
            if amount <= 0: 
                return await safe_reply(ctx, "❌ Poverty is not intimidating.", ephemeral=True)
            if ctx.author.id == target.id: 
                return await safe_reply(ctx, "❌ Attention seeking is a weakness.", ephemeral=True)
            
            u1_data = get_tower_data(ctx.author.id)
            if u1_data["gold"] < amount: 
                return await safe_reply(ctx, "❌ You lack the capital to back this threat.", ephemeral=True)
            
            if str(target.id) not in tower_db: 
                return await safe_reply(ctx, "❌ Target does not exist in the Tower.", ephemeral=True)
            ...
            desc = (
                f"**{ctx.author.display_name}** placed a bounty of **{amount} Gold** "
                f"on the head of **{target.display_name}**.\n\n"
                f"Total Bounty on Target: **{u2_data['bounty']} Gold**.\n"
                f"*Law 3: Conceal your intentions. Let others do your dirty work.*"
            )
            embed = discord.Embed(title="🎯 Vendetta Declared", description=desc, color=THEME_GOLD)
            await safe_reply(ctx, embed=embed)
        except Exception as e:
            traceback.print_exc()
            await safe_reply(ctx, f"⚠️ System Exception: {e}", ephemeral=True)
    ...
    async def tower_hierarchy(self, ctx):
        try:
            if not tower_db: 
                return await safe_reply(ctx, "The Tower is empty.", ephemeral=True)
            
            players = []
            for uid, data in tower_db.items():
                if isinstance(data, dict) and "floor" in data:
            ...
            desc = "Status is the only currency that matters."
            embed = discord.Embed(title="👑 The Court of Shadows", description=desc, color=THEME_BLOOD)
            
            c_lines = [f"<@{uid}>: Floor {d.get('floor', 0)}" for uid, d in top_climbers]
            embed.add_field(name="🧗 Highest Ascensions", value="\n".join(c_lines) or "None", inline=False)
            
            k_lines = [f"<@{u}>: {d.get('kills', 0)} Kills" for u, d in top_killers if d.get('kills', 0) > 0]
            embed.add_field(name="💀 Apex Predators", value="\n".join(k_lines) or "None", inline=False)
            
            w_lines = [f"<@{u}>: {d.get('bounty', 0)} Gold" for u, d in top_wanted if d.get('bounty', 0) > 0]
            if w_lines: 
                embed.add_field(name="🎯 Most Wanted", value="\n".join(w_lines), inline=False)
